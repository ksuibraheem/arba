...
class OfflineBufferService {
    ...
    async safeWrite(collectionPath, docId, data, operationType, firebaseWriter, serverTimestampFields = []) {
        // IMPORTANT: `data` passed in MUST be plain/JSON-serializable (no FieldValue
        // sentinels such as serverTimestamp()). Timestamp fields are named in
        // `serverTimestampFields` and stamped at write/replay time only.
        // If clearly offline, buffer immediately (store plain data + field names).
        if (!this._isOnline) {
     ...
     * Firestore serverTimestamp(). Kept async so firebase is only imported
     * when actually writing (so non-firebase environments / tests can use the
     * buffer logic without pulling in the SDK).
     */
    ...
    async syncPendingWrites(firebaseWriter) {
        ...
        for (const entry of buffer) {
            ...
            try {
                // Re-stamp serverTimestamp fields at replay time (never stored in JSON)
