...
const parsedDataPath = path.join(__dirname, '..', 'training_data', 'pending', 'parsed_data.json');
const parsedData = JSON.parse(fs.existsSync(parsedDataPath) ? fs.readFileSync(parsedDataPath, 'utf8') : (fs.existsSync('parsed_data.json') ? fs.readFileSync('parsed_data.json', 'utf8') : '{"sheets":{}}'));
...
const reportPath = path.join(__dirname, '..', 'training_data', 'pending', 'arba_v2_report.md');
...
console.log(`📄 تم حفظ التقرير في ملف ${reportPath}`);
