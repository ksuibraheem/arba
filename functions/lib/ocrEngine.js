...
exports.extractImagesFromExcel = extractImagesFromExcel;
exports.scanImagesForBranding = scanImagesForBranding;
exports.isImageScannable = isImageScannable;
 ...
 */
const VISUAL_BRANDING_PATTERNS = [
    // Major Saudi firms
 ...
 */
function extractImagesFromExcel(workbook) {
    const images = [];
    try {
        ...
        if (workbook._media) {
            for (const media of workbook._media) {
 ...
 */
async function scanSingleImage(imageBuffer, imageIndex) {
    const detections = [];
    try {
        // Run OCR with Tesseract - support both Arabic and English
        const result = await tesseract_js_1.default.recognize(imageBuffer, 'eng+ara', {
 ...
 */
async function scanImagesForBranding(images) {
 ...
 */
function isImageScannable(buffer) {
