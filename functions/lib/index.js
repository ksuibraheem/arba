...
exports.verifyEmployeeCredentials = exports.trackAgentQuery = exports.verifyRFQCommission = exports.processRFQCommission = exports.processAutoRenewal = exports.checkSubscriptionExpiry = exports.brainWeightSync = exports.brainDailyDiagnostic = exports.brainHeartbeat = exports.scheduledSessionCleanup = exports.verifyTapCharge = exports.createTapCharge = exports.certifyProjectPrice = exports.getHealthStatus = exports.getMarketRates = exports.autoProcessFile = exports.sanitizeAndProcess = exports.parseAndScan = void 0;
 ...
 */
exports.parseAndScan = (0, https_1.onCall)({
    maxInstances: 10,
    timeoutSeconds: 180,
    memory: '1GiB',
    region: 'us-central1',
    ...
    if (!fileBase64 || !fileName) {
        throw new https_1.HttpsError('invalid-argument', 'File data and name are required');
    }
    try {
        ...
        const parseResult = await (0, fileParser_1.parseFile)(buffer, fileName);
        // 3. Scan for Competitor Fingerprints (text-based)
        const scanReport = (0, sanitizationEngine_1.scanForFingerprints)(parseResult.sheets, parseResult.metadata);
        // 4. OCR Visual De-branding Scan
        let ocrReport = null;
        const allImages = parseResult.embeddedImages.filter(ocrEngine_1.isImageScannable);
        if (allImages.length > 0) {
            try {
                ocrReport = await (0, ocrEngine_1.scanImagesForBranding)(allImages);
        ...
        throw new https_1.HttpsError('internal', `Failed to parse file: ${error.message}`);
    }
});
 ...
 */
exports.sanitizeAndProcess = (0, https_1.onCall)({
    maxInstances: 10,
    timeoutSeconds: 120,
    memory: '512MiB',
    region: 'us-central1',
    ...
    if (!scanId || !columnMappings) {
        throw new https_1.HttpsError('invalid-argument', 'Scan ID and column mappings are required');
    }
    // 1. Retrieve scan session
    const session = scanSessions.get(scanId);
    if (!session) {
        throw new https_1.HttpsError('not-found', 'Scan session expired or not found. Please re-upload the file.');
    }
    try {
        const sheetIndex = selectedSheet || 0;
        const sheet = session.sheets[sheetIndex];
        if (!sheet) {
            throw new https_1.HttpsError('invalid-argument', 'Selected sheet not found');
        }
        // 2. Execute "The Purge" if confirmed
        let processedSheets = session.sheets;
        let purgeReport = [];
        if (purgeConfirmed !== false) { // Default to purge
            const purgeResult = (0, sanitizationEngine_1.executePurge)(session.sheets, session.metadata, session.report.matches);
        ...
        const rawItems = (0, formulaEngine_1.mapRowsToItems)(targetSheet.rows, targetSheet.headers, columnMappings);
        if (rawItems.length === 0) {
            throw new https_1.HttpsError('invalid-argument', 'No valid items found after mapping. Check column assignments.');
        }
        ...
        const processedItems = (0, formulaEngine_1.processImportedItems)(rawItems, overheadConfig);
        // 6. Save to Firestore
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError('internal', `Processing failed: ${error.message}`);
    }
});
 ...
 */
exports.autoProcessFile = (0, https_1.onCall)({
    maxInstances: 10,
    timeoutSeconds: 300,
    memory: '1GiB',
    region: 'us-central1',
    ...
    try {
        ...
        const result = await (0, apiGateway_1.executeFullPipeline)(buffer, fileName, request.auth.uid, options || {});
        ...
        if (error instanceof https_1.HttpsError)
            throw error;
        throw new https_1.HttpsError('internal', `Auto-process failed: ${error.message}`);
    }
});
 ...
 */
exports.getMarketRates = (0, https_1.onCall)({
    maxInstances: 20,
    timeoutSeconds: 30,
    memory: '256MiB',
    region: 'us-central1',
    ...
    try {
        const rates = await (0, marketRatesEngine_1.queryRates)({
            category,
            subcategory,
            search,
            region,
        });
        const categories = (0, marketRatesEngine_1.getCategories)();
        return {
            success: true,
            rates: rates.map(r => ({
                id: r.id,
                ...
                nameEn: r.nameEn,
                unit: r.unit,
                rate: region ? r.rates[region] : r.rates.riyadh,
                rates: r.rates,
                currency: r.currency,
                source: r.source,
        ...
        throw new https_1.HttpsError('internal', `Market rates query failed: ${error.message}`);
    }
});
 ...
 */
exports.getHealthStatus = (0, https_1.onCall)({
    maxInstances: 5,
    timeoutSeconds: 15,
    memory: '256MiB',
    region: 'us-central1',
    ...
    return {
        ...
        engines: {
            gateway: (0, apiGateway_1.getGatewayStats)(),
            marketRates: (0, marketRatesEngine_1.getEngineStats)(),
 ...
 */
exports.scheduledSessionCleanup = (0, scheduler_1.onSchedule)({
    schedule: 'every 30 minutes',
    region: 'us-central1',
}, async () => {
    const cleaned = await (0, sessionManager_1.cleanupExpiredSessions)();
 ...
 */
exports.brainHeartbeat = (0, scheduler_1.onSchedule)({
 ...
 */
exports.brainDailyDiagnostic = (0, scheduler_1.onSchedule)({
 ...
 */
exports.brainWeightSync = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        for (const [key, values] of Object.entries(allWeights)) {
            globalWeights[key] = Math.round((values.reduce((s, v) => s + v, 0) / values.length) * 100) / 100;
        }
 ...
 */
exports.checkSubscriptionExpiry = (0, scheduler_1.onSchedule)({
    schedule: 'every 6 hours',
    region: 'us-central1',
}, async () => {
    ...
    try {
        ...
        const expired = await (0, subscriptionManager_1.getExpiredSubscriptions)();
        let downgraded = 0;
        for (const sub of expired) {
            const success = await (0, subscriptionManager_1.downgradeToFree)(sub.data.userId, sub.id);
            if (success)
                downgraded++;
        }
        // 2. تنبيه الاشتراكات القريبة من الانتهاء (3 أيام)
        const expiring = await (0, subscriptionManager_1.getExpiringSubscriptions)(72);
        let warned = 0;
        for (const sub of expiring) {
            ...
            if (daysLeft === 3 || daysLeft === 1) {
                await (0, subscriptionManager_1.sendExpiryWarning)(sub.data.userId, sub.data.plan, daysLeft);
                warned++;
            }
        }
 ...
 */
exports.processAutoRenewal = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        const expiring = await (0, subscriptionManager_1.getExpiringSubscriptions)(24);
                ...
                if (sub.data.savedCardToken) {
                    ...
                    if (tapSecretKey) {
                        ...
                        if (charge.status === 'CAPTURED') {
                            await (0, subscriptionManager_1.extendSubscription)(sub.data.userId, sub.id, sub.data.billingCycle);
                            // سجل الدفع
 ...
 */
exports.processRFQCommission = (0, https_1.onCall)({
    maxInstances: 10,
    timeoutSeconds: 30,
    memory: '256MiB',
    region: 'us-central1',
    ...
    if (!rfqId || !orderTotal || orderTotal <= 0) {
        throw new https_1.HttpsError('invalid-argument', 'rfqId و orderTotal مطلوبان');
    }
    // 1. التحقق من أن المورد هو صاحب الطلب
    const rfqDoc = await db.collection('supplierRFQs').doc(rfqId).get();
    if (!rfqDoc.exists) {
        throw new https_1.HttpsError('not-found', 'الطلب غير موجود');
    }
    const rfq = rfqDoc.data();
    if (rfq.supplierId !== request.auth.uid) {
        throw new https_1.HttpsError('permission-denied', 'غير مصرح لك بدفع عمولة هذا الطلب');
    }
    if (rfq.commissionPaid) {
        throw new https_1.HttpsError('already-exists', 'تم دفع العمولة مسبقاً');
    }
    // 2. حساب العمولة
    const commission = (0, subscriptionManager_1.calculateRFQCommission)(orderTotal);
    // 3. إنشاء Tap charge
    const tapSecretKey = process.env.TAP_SECRET_KEY || '';
    ...
    try {
        ...
        throw new https_1.HttpsError('internal', `فشل إنشاء عملية الدفع: ${error.message}`);
    }
});
/**
 ...
 */
exports.verifyRFQCommission = (0, https_1.onCall)({
    maxInstances: 10,
    timeoutSeconds: 30,
    memory: '256MiB',
    region: 'us-central1',
    ...
    if (!rfqId || !tapChargeId) {
        throw new https_1.HttpsError('invalid-argument', 'rfqId و tapChargeId مطلوبان');
    }
    // 1. التحقق من الطلب
    const rfqDoc = await db.collection('supplierRFQs').doc(rfqId).get();
    if (!rfqDoc.exists) {
    ...
    const rfq = rfqDoc.data();
    if (rfq.supplierId !== request.auth.uid) {
        throw new https_1.HttpsError('permission-denied', 'غير مصرح');
    }
    // 2. التحقق من Tap
    const tapSecretKey = process.env.TAP_SECRET_KEY || '';
    try {
        ...
        if (charge.status === 'CAPTURED') {
            ...
            const commission = (0, subscriptionManager_1.calculateRFQCommission)(rfq.orderTotal || charge.amount / 0.06);
            await (0, subscriptionManager_1.recordRFQPayment)(rfqId, rfq.supplierId, rfq.clientId, commission, tapChargeId);
            return {
                success: true,
                status: 'paid',
        ...
        throw new https_1.HttpsError('internal', `فشل التحقق: ${error.message}`);
    }
});
/**
 ...
 */
exports.trackAgentQuery = (0, https_1.onRequest)({
    cors: true,
    region: 'us-central1'
}, async (req, res) => {
    try {
 ...
 */
exports.verifyEmployeeCredentials = (0, https_1.onCall)({
    cors: true,
    maxInstances: 5
}, async (request) => {
    try {
        ...
        if (!employeeDoc) {
            ...
            if (employeesDoc.exists) {
                ...
                if (data) {
                    ...
                    const emp = employees.find(e => (e.employeeNumber === employeeNumberOrEmail || e.email === employeeNumberOrEmail)
        ...
        throw new https_1.HttpsError('internal', err.message || 'Internal server error');
    }
});
