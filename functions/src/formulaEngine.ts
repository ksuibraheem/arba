...
export interface RawImportedItem {
    name_ar: string;
    name_en?: string;
    description?: string;
    unit: string;
    ...
    labor: number;
    equipment: number;
    waste: number;       // As decimal: 0.05 = 5%
    category: string;
    sbc?: string;
    rate?: number;       // External rate (will be recalculated)
}
...
export interface ProcessedItem {
    ...
    // Calculated values (server-only computation)
    materialsCost: number;
    wastageCost: number;
    laborCost: number;
    equipmentCost: number;
    directCost: number;      // Materials*Wastage + Labor + Equipment
    overheadAmount: number;
    totalUnitCost: number;   // Direct × Overheads
    totalLinePrice: number;  // totalUnitCost × qty
...
export interface OverheadConfig {
    overheadMultiplier: number;      // e.g., 1.15 = 15% overhead
    profitMargin: number;            // e.g., 0.10 = 10% profit
    contingency: number;             // e.g., 0.05 = 5% contingency
}
 ...
 */
export function processImportedItems(
    ...
    return rawItems.map((item, index) => {
        ...
        return {
            ...
            importedAt: now,
            source: 'arba_universal_parser' as const,
        };
    });
}
...
export function mapRowsToItems(
    ...
    for (const row of rows) {
        const getValue = (field: string): any => {
            ...
            if (idx === undefined || idx >= (row?.length || 0)) return undefined;
            return row[idx];
        };

        const item: RawImportedItem = {
        ...
        // Skip empty rows
        if (!item.name_ar && !item.name_en) continue;

        // Fallback: copy name between AR/EN
        if (!item.name_ar && item.name_en) item.name_ar = item.name_en;
 ...
 */
function normalizeWaste(value: any): number {
