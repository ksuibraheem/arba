...
import { parseFile, autoMapColumns, ARBA_SCHEMA_FIELDS, ParsedFileResult } from './fileParser';
import { scanForFingerprints, executePurge, applyWhiteLabel, SanitizationReport } from './sanitizationEngine';
import { scanImagesForBranding, isImageScannable } from './ocrEngine';
import { mapRowsToItems, processImportedItems, OverheadConfig, ProcessedItem } from './formulaEngine';
import { createSession, updateSession, deleteSession, SessionData } from './sessionManager';
import { compareWithMarket, MarketComparison } from './marketRatesEngine';
...
export interface PipelineStage {
    name: 'parse' | 'scan' | 'ocr' | 'purge' | 'map' | 'calculate' | 'market_compare' | 'save';
...
export interface PipelineOptions {
    autoPurge?: boolean;          // default: true for CLEAN/WARNING
    overheadConfig?: OverheadConfig;
    injectMarketRates?: boolean;  // default: false
    marketRegion?: 'riyadh' | 'jeddah' | 'dammam';
    selectedSheet?: number;       // default: 0
}
...
export interface AutoProcessResult {
    ...
    // Security report (always returned)
    securityReport: {
    ...
    // Only returned when mode = 'auto_complete'
    items?: ProcessedItem[];
    itemCount?: number;
    totalValue?: number;
...
export async function executeFullPipeline(
    ...
    try {
        parseResult = await parseFile(fileBuffer, fileName);
        stages.push({
            name: 'parse',
            status: 'success',
    ...
    let scanReport: SanitizationReport;
    ...
    const scannableImages = parseResult.embeddedImages.filter(isImageScannable);
    let ocrDetectionCount = 0;

    if (scannableImages.length > 0) {
        try {
            const ocrReport = await scanImagesForBranding(scannableImages);
    ...
    const rawItems = mapRowsToItems(targetSheet.rows, targetSheet.headers, autoMappings);
    stages.push({
        name: 'map',
    ...
    try {
        processedItems = processImportedItems(rawItems, overheadConfig);
        stages.push({
            name: 'calculate',
            status: 'success',
    ...
    if (options.injectMarketRates) {
        try {
            ...
            marketComparison = await compareWithMarket(
...
function buildFailedResult(
    ...
    securityReport?: AutoProcessResult['securityReport']
): AutoProcessResult {
    return {
        success: false,
        mode: 'auto_complete',
