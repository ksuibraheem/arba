...
import { FileMetadata, ParsedSheet } from './fileParser';
import { OcrScanReport } from './ocrEngine';
...
export interface SanitizationReport {
    ...
    headerFlags: string[];
    isClean: boolean;
    ocrReport?: OcrScanReport;
    securityAlertLevel: 'BLOCKED' | 'WARNING' | 'CLEAN';
}

export interface PurgeResult {
    sanitizedSheets: ParsedSheet[];
    sanitizedMetadata: FileMetadata;
    removedItems: string[];
    arbaStamped: boolean;
    customPropertiesStripped: number;
 ...
 */
const COMPETITOR_PATTERNS: { pattern: RegExp; name: string; category: string; severity: 'critical' | 'warning' | 'info' }[] = [
...
export function scanForFingerprints(
    ...
    for (const sheet of sheets) {
        ...
        sheet.rows.forEach((row, rowIdx) => {
            (row || []).forEach((cell, colIdx) => {
                ...
                if (cellStr.trim().length === 0) return;
    ...
    const securityAlertLevel: 'BLOCKED' | 'WARNING' | 'CLEAN' =
        criticalCount > 0 ? 'BLOCKED' :
...
export function executePurge(
    sheets: ParsedSheet[],
    metadata: FileMetadata,
    _confirmedMatches: CompetitorMatch[]
): PurgeResult {
    ...
    // 2. Sanitize sheets
    const sanitizedSheets: ParsedSheet[] = sheets.map(sheet => {
        // Sanitize headers
 ...
 */
export function applyWhiteLabel(sheets: ParsedSheet[]): ParsedSheet[] {
