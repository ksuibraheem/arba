...
import { ChevronLeft, Plus, Users, Trash2, CheckCircle, XCircle, Search, Shield, MessageCircle, Camera, FileText, ClipboardList, BarChart3, UserPlus, X } from 'lucide-react';
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className="text-[10px] text-violet-400 bg-violet-500/10 px-1.5 py-0.5 rounded">{roleInfo[language === 'ar' ? 'ar' : 'en']}</span>
                                ...
                                <select value={member.role} onChange={e => handleChangeRole(member, e.target.value as ProjectRole)}
                                    ...
                                    {(Object.keys(PROJECT_ROLES) as ProjectRole[]).map(role => (
                                        <option key={role} value={role}>
                                            {PROJECT_ROLES[role].icon} {PROJECT_ROLES[role][language === 'ar' ? 'ar' : 'en']}
                                        </option>
                                    ))}
                                </select>
                            </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div>
                                ...
                                <select value={newRole} onChange={e => setNewRole(e.target.value as ProjectRole)}
                                    ...
                                    {(Object.keys(PROJECT_ROLES) as ProjectRole[]).map(role => (
                                        <option key={role} value={role}>{PROJECT_ROLES[role].icon} {PROJECT_ROLES[role][language === 'ar' ? 'ar' : 'en']}</option>
                                    ))}
                                </select>
                            </div>
