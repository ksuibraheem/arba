...
for (const sheetName of workbook.SheetNames) {
  ...
  for (let i = 0; i < jsonData.length; i++) {
    ...
    if (!unit || desc.length < 5) continue;
    if (unit.includes('سعر') || unit.includes('معدل') || unit === 'الوحدة') continue;
    
    allItems.push({ sheet: sheetName, row: i, unit, desc });
  }
}

// Define ALL categories the brain should recognize
const CATEGORIES = {
  // Earthworks
  excavation: ['حفر', 'خنادق', 'تسوية'],
  backfill: ['ردم', 'دك', 'تعويض', 'granual'],
  antiTermite: ['نمل أبيض', 'نمل'],
  dewatering: ['نزح', 'مياه أرضية'],
  shoring: ['دعامات', 'سند جوانب'],
  
  // Concrete
  plainConcrete: ['خرسانة عادية', 'فرشات نظافة', 'فرشة نظافة', 'blinding'],
  rc_slabOnGrade: ['بلاطة أرضية', 'slab on grade', 'sog', 'بلاطات أرضية'],
  rc_footings: ['قواعد', 'footings', 'أساسات منفصلة'],
  rc_raft: ['لبشة', 'حصيرة', 'raft', 'mat foundation'],
  rc_walls: ['حوائط', 'جدران خرسانة', 'حائط الخزان', 'retaining'],
  rc_columns: ['أعمدة', 'رقاب', 'columns'],
  rc_beams: ['كمرات', 'beams', 'الكمر'],
  rc_slabs: ['بلاطات', 'slabs', 'أسقف خرسانة'],
  rc_stairs: ['درج', 'منحدرات', 'stairs', 'ramp'],
  rc_tanks: ['خزان', 'tank'],
  
  // Masonry
  blockwork: ['بلك', 'بلوك', 'بناء طوب', 'مباني', 'block'],
  ...
  graniteRamp: ['منحدر.*جرانيت', 'محزز'],
  terrazzo: ['موزايكو', 'terrazzo'],
  vinyl: ['فينايل', 'vinyl', 'vct'],
  vinylSkirting: ['وزرة فينايل'],
  carpet: ['موكيت', 'carpet'],
  epoxy: ['بولي يوريثان', 'ايبوكسي', 'epoxy', 'polyurethane'],
  ...
  waterproofing: ['عزل مائي', 'waterproofing', 'bitumen', 'membrane'],
  ...
  primeCost: ['prime coat', 'لاصقة.*إسفلت'],
...
// Check each item against categories
const covered = [];
const uncovered = [];
...
console.log(`Covered by brain: ${covered.length}`);
console.log(`UNCOVERED (GAPS): ${uncovered.length}`);
console.log(`Coverage: ${((covered.length / allItems.length) * 100).toFixed(1)}%`);
console.log();

if (uncovered.length > 0) {
  console.log(`=== UNCOVERED ITEMS (NEED TO ADD) ===`);
  uncovered.forEach((item, idx) => {
    console.log(`${idx + 1}. [${item.unit}] ${item.desc.substring(0, 120)}`);
  });
}
