 ...
 * ┘è╪ص╪د┘â┘è: ┘é╪╖╪╣ ╪د┘╪ح┘╪ز╪▒┘╪ز╪î ╪ح╪╣╪د╪»╪ر ╪ح╪▒╪│╪د┘ ╪د┘╪╖╪د╪ذ┘ê╪▒╪î ╪│┘╪د┘à╪ر ╪د┘╪ز╪│┘╪│┘ (serverTimestamp)╪î
 * ┘ê╪│┘┘ê┘â ╪د┘╪ص┘╪╕ ╪د┘╪ز┘┘é╪د╪خ┘è ┘┘┘à╪│┘ê╪»╪ر.
 */
class MemoryStorage {
    m = new Map();
...
async function main() {
    ...
    const res1 = await offlineBufferService.safeWrite('projects', 'proj_1', { name: '┘┘è┘╪د', estimatedValue: 100 }, 'update', recordingWriter, ['updatedAt']);
    ...
    await offlineBufferService.safeWrite('projects', 'proj_2', { name: '╪ذ╪▒╪ش' }, 'create', recordingWriter, ['createdAt', 'updatedAt']);
