...
const RegisterPage: React.FC<RegisterPageProps> = ({ language, onNavigate, onRegister }) => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#070914] via-[#0E132B] to-[#0A1020] py-6 sm:py-10 px-4 sm:px-6" dir={isRtl ? 'rtl' : 'ltr'}>
            {/* Background Pattern */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                ...
                <div className="absolute -bottom-1/2 -right-1/2 w-full h-full bg-gradient-to-tr from-lime-500/10 to-transparent rounded-full blur-3xl"></div>
            </div>
            ...
            <div className="relative w-full max-w-2xl mx-auto">
                ...
                {/* Register Card */}
                <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl sm:rounded-3xl border border-slate-700/50 p-5 sm:p-8 shadow-2xl">
                    {/* Logo */}
                    <div className="flex flex-col items-center mb-6 sm:mb-8">
                        <ArbaLogo size={56} animated className="mb-3" />
                        <h1 className="text-xl sm:text-2xl font-bold text-white mb-1 sm:mb-2">{t('register_title')}</h1>
                        <p className="text-sm sm:text-base text-slate-400">{COMPANY_INFO.systemName[language]}</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                                ...
                                {userTypes.map((type) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`relative p-3 sm:p-4 rounded-lg sm:rounded-xl border transition-all flex flex-col items-center gap-1.5 sm:gap-2 ${userType === type.id
                                                ? 'bg-green-500/20 border-green-500 text-white'
                                                : 'bg-slate-700/30 border-slate-600 text-slate-400 hover:border-slate-500'
                                                }`}
                                        >
                                            <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                                            <span className="font-medium text-xs sm:text-sm">{type.label[language]}</span>
                                            {userType === type.id && (
                                                <Check className="absolute top-2 end-2 w-4 h-4 text-green-400" />
                                            )}
                                        </button>
                                    );
                                })}
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <select
                                            ...
                                            className="w-full bg-slate-700/50 border border-slate-600 rounded-xl py-3 ps-12 pe-4 text-white placeholder-slate-500 focus:outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all appearance-none"
                                            required
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full py-4 bg-gradient-to-r from-green-500 to-lime-500 hover:from-green-400 hover:to-lime-400 text-white rounded-xl font-bold text-lg transition-all shadow-lg shadow-green-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {loading ? (
